...
elif bc_model_path_train == "./bc_runs_ireca/reproduce_train/cramped_room_tomato":
    policy_optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate_policy_cr)
    value_optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate_value_cr)
...
#         action_logits_bc_agent = bc_model.predict_on_batch(observation_HM)
        ...
        ###############################################
        if t % 100 == 0:
            #print(obs_dict_new["both_agent_obs"][1-other_agent_env_idx])
            img = env.render()

            plt.imshow(img)
            plt.axis("off")
            plt.pause(0.1)  # Pause for a short time to visualize

            if done:
                print(f"Episode finished after {episode_length} timesteps")
                break
        ############################
